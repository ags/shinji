...
describe Shinji::PayloadRedactor do
  describe "#redact" do
    ...
    it "replaces SQL containing sensitive data with [REDACTED]" do
      expect(redacted_payload.sql_events.first.payload[:sql]).to eq("SELECT * FROM posts")
      expect(redacted_payload.sql_events.last.payload[:sql]).to eq("[REDACTED]")
      expect(transaction_payload.sql_events.last.payload[:sql]).to eq("[REDACTED]")
    end
